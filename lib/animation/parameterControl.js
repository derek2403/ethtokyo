...
function forceMouthVisibility(model, formValue = 0.65) {
  if (!model) return;
...
export function applyAnimationParameters(animationState, model) {
  ...
  // CRITICAL: Force mouth visibility FIRST to ensure stable mouth shape
  forceMouthVisibility(model, 0.65);
  ...
  // Mouth opening parameter: use base + current for additive control
...
export function applyAnimationParametersAdditive(animationState, model) {
  ...
  try {
    ...
    // Force mouth form first (always override)
    forceMouthVisibility(model, 0.65);
