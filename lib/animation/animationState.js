...
export function initializeAnimationState(model) {
  if (!animationState.initialized && model?.internalModel?.coreModel) {
    try {
      ...
      animationState.baseMouthOpen = get('ParamMouthOpenY', 0) ?? 0;
      animationState.baseMouthForm = get('ParamMouthForm', 0.5) ?? 0.5;
      animationState.mouthCurrent = animationState.baseMouthOpen;
