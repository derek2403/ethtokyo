...
function setupModelDisplay(model, app) {
  ...
  const scaleX = (app.screen.width * 0.7) / modelBounds.width;
  const scaleY = (app.screen.height * 0.9) / modelBounds.height;
...
export function updateModelDisplay(model, app) {
  ...
  try {
    ...
    const scaleX = (app.screen.width * 0.7) / originalWidth;
    const scaleY = (app.screen.height * 0.9) / originalHeight;
